<template>
  <div>
    <div
      class="fixed w-[150%] h-[200%]  z-[0] grid 2xl:grid-cols-[repeat(6,1fr)] xl:grid-cols-[repeat(4,1fr)] lg:grid-cols-[repeat(4,1fr)] md:grid-cols-[repeat(3,1fr)] grid-cols-[repeat(3,1fr)] left-0 top-0 js-grid"
    >
      <div class="relative" v-for="(src, index) in images" :key="index">
        <figure
          class="js-plane inset-2 absolute m-0 p-0"
          :data-src="src"
        ></figure>
      </div>
    </div>
    <div class="page js-page"></div>
    <div class="DOM w-screen h-screen">
      <img class="rounded-3xl object-cover" />
    </div>
  </div>
  <div class="BUTTON fixed bottom-[15%] opacity-0 w-full flex justify-center">
    <button class="text-[#090909] text-lg cursor-pointer  transition-all duration-[0.3s] shadow-[6px_6px_12px_#c5c5c5,-6px_-6px_12px_#ffffff] px-[1.7em] py-[0.7em] rounded-[0.5em] border-solid border-[#e8e8e8] active:text-[#666] active:shadow-[inset_4px_4px_12px_#c5c5c5,inset_-4px_-4px_12px_#ffffff]">Click me</button>
  </div>
</template>
        ...
        onClick = (event: MouseEvent) => {
          ...
          if (intersects.length > 0) {
            ...
            if (plane) {
              ...
              const screenX =
                ((vector.x + 1) / 2) * window.innerWidth - width / 2;
              const screenY =
